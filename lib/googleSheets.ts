...
// --- Configuration (Updated with proper quoting) ---
...
// Fix: Added quotes around sheet name with spaces
...
const normalizeActivity = (row: any[], rowIndex: number): Activity | null => {
    ...
    return { 
        id: id, 
        name: name, 
        location: row[3]?.toString().trim() || 'N/A', 
        category: row[4]?.toString().trim() || 'General', 
        description: row[5]?.toString().trim() || 'No description.', 
        registrationLink: row[6]?.toString().trim() || '#', 
        imageURL: row[7]?.toString().trim() || null, 
        activityDate: row[8]?.toString().trim() || null, 
        ageRange: row[9]?.toString().trim() || 'N/A' 
    };
};
...
export async function getActivities(): Promise<Activity[]> {
    ...
    try {
        ...
        if (err.code === 404) { 
            ...
            specificError = `Activities sheet tab ('${ACTIVITIES_SHEET_NAME}') not found or range invalid ('${ACTIVITIES_RANGE}').`; 
        }
